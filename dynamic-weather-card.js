...
// src/internationalization/locales/ru/translation.json
...
// src/internationalization/locales/de/translation.json
...
// src/internationalization/locales/nl/translation.json
...
// src/internationalization/locales/fr/translation.json
...
// src/internationalization/locales/en/translation.json
...
// src/internationalization/locales/es/translation.json
...
// src/internationalization/locales/it/translation.json
...
// src/internationalization/locales/sk/translation.json
...
// src/internationalization/locales/hu/translation.json
...
var translations = {
  ...
  sk: translation_default8,
  hu: translation_default9
};
...
function formatForecastTime(datetime, format = "24h", am = "AM", pm = "PM") {
  if (!datetime)
    return "";
...
// src/components/animation-manager.ts
...
// src/components/forecast-service.ts
class ForecastService {
  hourlyForecast = [];
  dailyForecast = [];
  hourlySubscription = null;
  ...
  async subscribe(hass, entityId, showDaily) {
    if (!hass || !entityId) {
      return;
    }
    this.unsubscribe();
    try {
      this.hourlySubscription = hass.connection.subscribeMessage((event) => {
      ...
      if (showDaily) {
        this.dailySubscription = hass.connection.subscribeMessage((event) => {
          if (event.forecast && event.forecast.length > 0) {
  ...
  getHourlyForecast(hours, fallbackWeatherData) {
    ...
    return todayForecast.sort((a, b) => new Date(a.datetime).getTime() - new Date(b.datetime).getTime()).slice(0, maxHours);
  }
  getDailyForecast(days, fallbackWeatherData) {
    ...
    return Array.from(dayBuckets.values()).sort((a, b) => a.itemDate.getTime() - b.itemDate.getTime()).map((entry) => entry.item).slice(0, maxDays);
  }
}
...
class ActionHandler {
  ...
  getConfig;
  fireEvent;
  constructor(getHass, getConfig, fireEvent) {
...
// src/components/weather-data.ts
function getWeatherState(hass, entityId) {
  if (!hass || !entityId)
    return null;
  const entity = hass.states[entityId];
  return entity ? entity.state : null;
}
function getWeatherAttributes(hass, entityId) {
  if (!hass || !entityId)
    return {};
  const entity = hass.states[entityId];
  return entity ? entity.attributes : {};
}
function getWeatherData(hass, entityId, config, hourlyForecast) {
  const state3 = getWeatherState(hass, entityId);
  ...
  if (config.templowAttribute && attrs[config.templowAttribute] != null) {
    templow = attrs[config.templowAttribute];
  } else {
...
__legacyDecorateClassTS([
  property({ type: String })
], HourlyForecast.prototype, "clockFormat", undefined);
...
class AnimatedWeatherCard extends LitElement {
  ...
  constructor() {
    ...
    this.actionHandler = new ActionHandler(() => this.hass, () => this.config, (type, detail) => this.fireEvent(type, detail));
  }
  connectedCallback() {
    super.connectedCallback();
    this.updateComplete.then(() => {
  ...
  getDrawParams() {
    if (!this.hass || !this.config.entity)
      return null;
    const weather = getWeatherData(this.hass, this.config.entity, this.config, this.forecastService.getHourlyData());
  ...
  render() {
    ...
    const hourlyForecast = this.config.showHourlyForecast ? this.forecastService.getHourlyForecast(this.config.hourlyForecastHours ?? DEFAULT_CONFIG.hourlyForecastHours, weather) : [];
    const dailyForecast = this.config.showDailyForecast ? this.forecastService.getDailyForecast(this.config.dailyForecastDays ?? DEFAULT_CONFIG.dailyForecastDays, weather) : [];
    return html`
      <ha-card
        @click=${(e) => this.actionHandler.handleTap(e)}
        @pointerdown=${() => this.actionHandler.handlePointerDown()}
        @pointerup=${(e) => this.actionHandler.handlePointerUp(e)}
        @pointercancel=${(e) => this.actionHandler.handlePointerUp(e)}
      >
