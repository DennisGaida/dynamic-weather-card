...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:i,strings:o,values:a}},_=Ni(ja),k=Ni(ri),wn=Ni(si),W=Symbol.for("lit-noChange"),z=Symbol.for("lit-nothing"),ho=new WeakMap,B=J.createTreeWalker(J,129),q=di;function wo(i,o){if(!ji(i)||!i.hasOwnProperty("raw")){let a="invalid template strings array";throw a=`
...
`,ln={sunny:k`
  ...
  `;hasContent(){if(!this.weather||!this.config)return!1;return this.config.showHumidity&&this.weather.humidity!=null||this.config.showWind&&this.weather.windSpeed!=null||this.config.showSunriseSunset&&this.sunData?.hasSunData===!0}renderHumidity(){if(!this.config?.showHumidity||this.weather?.humidity==null)return _``;return _`
      <div class="info-item">
        <span class="info-icon">${oi("humidity")}</span>
        <span>${this.weather.humidity} %</span>
      </div>
    `}renderSunrise(){if(!this.config?.showSunriseSunset||!this.sunData?.hasSunData||!this.sunData.sunrise)return _``;return _`
      <div class="info-item">
        <span class="info-icon">${oi("sunrise")}</span>
        <span>${Di(this.sunData.sunrise,this.config.clockFormat,d.t("am"),d.t("pm"))}</span>
      </div>
    `}renderWind(){if(!this.config?.showWind||this.weather?.windSpeed==null)return _``;let i=this.entityAttributes||{},o=Bi(this.weather.windSpeed,i,this.config.windSpeedUnit),a=Uo(i,this.config.windSpeedUnit,d.t.bind(d)),n="";if(this.config.showWindGust&&this.weather.windGust)n=` / ${Bi(this.weather.windGust,i,this.config.windSpeedUnit)} ${a}`;let e=this.config.showWindDirection&&this.weather.windBearing!=null?Oo(this.weather.windBearing):oi("wind");return _`
      <div class="info-item">
        <span class="info-icon">${e}</span>
        <span>${o} ${a}${n}</span>
      </div>
    `}renderSunset(){if(!this.config?.showSunriseSunset||!this.sunData?.hasSunData||!this.sunData.sunset)return _``;return _`
      <div class="info-item">
        <span class="info-icon">${oi("sunset")}</span>
        <span>${Di(this.sunData.sunset,this.config.clockFormat,d.t("am"),d.t("pm"))}</span>
      </div>
    `}render(){if(!this.hasContent())return _``;return _`
      <div class="info-grid">
        ${this.renderHumidity()}
        ${this.renderWind()}
        ${this.renderSunrise()}
...
`;class Eo extends v{constructor(){super(...arguments);this.forecast=[];this.clockFormat="24h"}static styles=ki;_cleanup=null;connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{this._cleanup=gi(this.shadowRoot,".forecast-scroll")})}disconnectedCallback(){super.disconnectedCallback(),this._cleanup?.(),this._cleanup=null}getTemperature(i){return Math.round(i.temperature??i.temp??i.native_temperature??0)}render(){if(this.forecast.length===0)return _``;return _`
    ...
    `}renderDefault(i,o,a,n,e,l,r){return _`
      <div class="${l}" style="${r}">
        ${this.config.showAnimations!==!1?_`<div class="canvas-container"></div>`:""}
        <div class="content">
          ${this.config.name&&this.config.name.trim()!==""?_`
            <div class="header">
    ...
    `}renderMinimal(i,o,a,n,e){let l=i.temperature!=null?Math.round(i.temperature)+"°":d.t("no_data"),r=i.templow!=null?`↓ ${Math.round(i.templow)}°`:null;return _`
      <div class="${n} layout--minimal" style="${e}">
        ${this.config.showAnimations!==!1?_`<div class="canvas-container"></div>`:""}
        <div class="content">
          <div class="mini-primary">
            <div class="mini-temp">${l}</div>
            ${this.config.showMinTemp&&r?_`<div class="mini-temp-low">${r}</div>`:""}
          </div>
